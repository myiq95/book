...
/* =================================================================
   네이티브(Capacitor) 앱에서의 TTS
   ------------------------------------------------------------------
   웹 버전은 화면이 잠기면 iOS가 speechSynthesis를 강제로 멈춰버려서
   백그라운드 재생이 근본적으로 불가능하다. Capacitor로 감싼 네이티브
   앱에서는 iOS 기본 읽어주기 엔진을 직접 쓰는 TTSPlugin(Swift)이
   있으므로, 그쪽을 대신 호출해 화면이 꺼지거나 잠겨도 계속 읽어준다.
   요청대로 기능은 단순하게: 챕터 전체를 순서대로 읽고, 재생·일시정지·
...
function setupNativeTtsBridge(){
  ...
  // 네이티브에서는 문장 단위 건너뛰기를 지원하지 않으므로 관련 버튼을 숨긴다.
  const prev = $('#ttsPrev'), next = $('#ttsNext');
  ...
  nativeTts().addListener('finished', ()=>{
    ...
    if(State.current.chapter < State.current.book.chapters.length-1){
      ...
      requestAnimationFrame(()=> speakChapterNative());
    } else {
      stopTTS(); toast('읽기 완료');
    }
  });
  nativeTts().addListener('boundary', (data)=>{
    if(!data || !data.text) return;
    el.ttsStatus.textContent = data.text.slice(0,30) + (data.text.length>30?'…':'');
  });
}
function speakChapterNative(){
...
function ttsJump(dir){ if(!State.tts.speaking || isNativeApp()) return; if(State.tts.synth) State.tts.synth.cancel(); State.tts.idx = Math.max(0, State.tts.idx + dir); speakNext(); }
