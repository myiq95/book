...
ios/App/App/MainViewController.swift  TTSPlugin을 브릿지에 등록하는 커스텀 뷰컨트롤러
ios/App/App/Info.plist        UIBackgroundModes(audio) 추가됨
.github/workflows/build-ipa.yml   GitHub Actions 자동 빌드
```

Capacitor iOS는 `8.0.1`로 고정했습니다. `8.5` 이상은 UIScene 아키텍처로
크게 리팩토링되면서 `CAPBridgeViewController`의 `bridge` 프로퍼티 접근
방식이 바뀌어(정확한 대체 API가 공식 문서에 아직 명확히 나와 있지 않음)
로컬 플러그인 등록이 실패했습니다. `8.0.1`은 이 방식(Main.storyboard의
Bridge View Controller가 `MainViewController`를 커스텀 클래스로 쓰고,
`capacitorDidLoad()`에서 `bridge?.registerPluginInstance(...)`로 등록)이
검증된 마지막 안정 버전대입니다.
